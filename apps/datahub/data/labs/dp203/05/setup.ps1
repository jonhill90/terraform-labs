...
# This script is intended for lab 05 of the DP-203 course
...
function Download-GitHubFiles {
    param(
        ...
        [string]$SourcePath = "Allfiles/labs/05",
...
if (-not $SkipAzureCopy) {
    ...
    # Create directory in the bronze container for CSV files
    Write-Host "Creating destination directories in the bronze container..." -ForegroundColor Cyan
    
    # Create directory for Lab 05 data
    try {
        az storage fs directory create --account-name $StorageAccountName --file-system bronze --name "dp203/05/sales/orders" --auth-mode login
        Write-Host "Created directory: dp203/05/sales/orders" -ForegroundColor Green
    }
    catch {
    ...
    foreach ($file in $csvFiles) {
        ...
        if (Test-Path $filePath) {
            try {
                az storage fs file upload --account-name $StorageAccountName --file-system bronze --path "dp203/05/sales/orders/$file" --source $filePath --auth-mode login
                Write-Host "Uploaded $file to dp203/05/sales/orders" -ForegroundColor Green
            }
            catch {
