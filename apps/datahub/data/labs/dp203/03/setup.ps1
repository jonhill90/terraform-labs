...
# This script is intended for lab 03 of the DP-203 course
...
function Download-GitHubFiles {
    param(
        ...
        [string]$SourcePath = "Allfiles/labs/03",
...
if (-not $SkipAzureCopy) {
    ...
    # Create directories in the bronze container for CSV files
    Write-Host "Creating destination directories in the bronze container..." -ForegroundColor Cyan
    
    # Create directory for CSV files
    try {
        az storage fs directory create --account-name $StorageAccountName --file-system bronze --name "dp203/03/sales/csv" --auth-mode login
        Write-Host "Created directory: dp203/03/sales/csv" -ForegroundColor Green
    }
    catch {
        Write-Error "Error creating sales/csv directory: $_"
    ...
    foreach ($file in $csvFiles) {
        ...
        if (Test-Path $filePath) {
            try {
                az storage fs file upload --account-name $StorageAccountName --file-system bronze --path "dp203/03/sales/csv/$file" --source $filePath --auth-mode login
                Write-Host "Uploaded $file to dp203/03/sales/csv" -ForegroundColor Green
            }
            catch {
