# PowerShell script to set up Azure Synapse Analytics for lab 11 of the DP-203 course
# This script connects to Azure, downloads necessary files from GitHub,
# and uploads CSV files to Azure Data Lake Storage.
...
function Download-GitHubFiles {
    param(
        ...
        [string]$SourcePath = "Allfiles/labs/11",
    ...
    $dataDirectory = "$DestinationPath/data"
    ...
    foreach ($file in $csvFiles) {
        ...
        $destination = "$dataDirectory/$file"
        try {
...
if ($TestConnection) {
    try {
        Write-Host "Testing connection to Azure..." -ForegroundColor Cyan
...
if (-not $SkipAzureCopy) {
    ...
    # Create directory structure in the bronze container
    Write-Host "Setting up directory structure in the bronze container..." -ForegroundColor Cyan
    
    # Create dp203/11/data directory
    try {
        az storage fs directory create --account-name $StorageAccountName --file-system bronze --name "dp203/11/data" --auth-mode login
        Write-Host "Created directory: dp203/11/data" -ForegroundColor Green
    }
    catch {
    ...
    foreach ($file in $csvFiles) {
        ...
        if (Test-Path $filePath) {
            try {
                az storage fs file upload --account-name $StorageAccountName --file-system bronze --path "dp203/11/data/$file" --source $filePath --auth-mode login
                Write-Host "Uploaded $file to dp203/11/data/" -ForegroundColor Green
            }
            catch {
...
OPTIONS:
    -TestConnection    : Test the Azure connection and exit
...
EXAMPLES:
    ...
    .\setup.ps1 -TestConnection                      : Test the Azure connection only
