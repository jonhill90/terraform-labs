# PowerShell script to set up Azure Synapse Analytics for lab 10 of the DP-203 course
# This script connects to Azure, downloads necessary files from GitHub,
...
function Download-GitHubFiles {
    param(
        ...
        [string]$SourcePath = "Allfiles/labs/10",
    ...
    try {
        ...
        # Try to create a basic setup.sql if the download fails
        $basicSetupSql = @"
SET ANSI_NULLS ON
GO
...
GO
"@
        $basicSetupSql | Out-File -FilePath $setupSqlDestination -Encoding utf8
        Write-Host "Created basic setup.sql file" -ForegroundColor Yellow
    }
    
    # Download setup.json if it exists
    $setupJsonUrl = "$baseUrl/setup.json"
    $setupJsonDestination = "$DestinationPath/setup.json"
    try {
        Invoke-WebRequest -Uri $setupJsonUrl -OutFile $setupJsonDestination
        Write-Host "Downloaded: setup.json" -ForegroundColor Green
    } catch {
        Write-Error "Failed to download setup.json: $_"
    }
    
    # Download CSV files - Product.csv is expected for Lab 10
    $csvFiles = @(
        "Product.csv"
    )
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    # Create a sample Product.csv file if download fails
                    if ($file -eq "Product.csv") {
                        $sampleProductCsv = @"
ProductAltKey,ProductName,Color,Size,ListPrice,Discontinued
AR-5381,Adjustable Race,Red,,1.99,0
BA-8327,Bearing Ball,Silver,1.5,2.99,0
BE-2349,BB Ball Bearing,Black,2.0,3.49,0
BE-2908,Headset Ball Bearings,Silver,1.0,4.99,0
BL-2036,Blade,Silver,,3.99,0
"@
                        $sampleProductCsv | Out-File -FilePath $destination -Encoding utf8
                        Write-Host "Created sample Product.csv file" -ForegroundColor Yellow
                    }
                }
            }
        }
...
if (-not $SkipAzureCopy) {
    ...
    # Create dp203/10/data directory
    try {
        az storage fs directory create --account-name $StorageAccountName --file-system bronze --name "dp203/10/data" --auth-mode login
        Write-Host "Created directory: dp203/10/data" -ForegroundColor Green
    }
    catch {
    ...
    if ($csvFiles.Count -eq 0) {
        ...
        foreach ($file in $csvFiles) {
            ...
            try {
                az storage fs file upload --account-name $StorageAccountName --file-system bronze --path "dp203/10/data/$fileName" --source $filePath --auth-mode login
                Write-Host "Uploaded $fileName to dp203/10/data" -ForegroundColor Green
                $successFiles++
            }
            catch {
