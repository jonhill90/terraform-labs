...
# First check and drop view if it exists
if ($TestConnection -eq $false -and $ListTables -eq $false) {
    try {
        $viewQuery = @"
IF OBJECT_ID('dbo.vFactSales', 'V') IS NOT NULL 
    DROP VIEW [dbo].[vFactSales];
"@
        Invoke-Sqlcmd -ServerInstance $SqlServer -Database $SqlDatabase -Username $SqlUser -Password $SqlPassword -Query $viewQuery -ErrorAction Stop
        Write-Host "Dropped view: vFactSales (if it existed)" -ForegroundColor Green
    }
    catch {
        Write-Error "Error dropping view: $_"
    }
}

# List of tables to drop, in the correct order to avoid foreign key constraints
# First drop the fact tables, then dimension tables, finally staging tables
$tables = @(
    # Fact tables first (they have foreign keys to dimension tables)
    "FactInternetSales",
    "FactResellerSales",
