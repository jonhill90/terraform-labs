...
# This script is intended for lab 07 of the DP-203 course
...
function Download-GitHubFiles {
    param(
        ...
        [string]$SourcePath = "Allfiles/labs/07",
    ...
    $url = "$baseUrl/data/products.csv"
    $destination = "$dataDirPath/products.csv"
...
if (-not $SkipAzureCopy) {
    ...
    # Create directory in the bronze container for data files
    Write-Host "Creating destination directories in the bronze container..." -ForegroundColor Cyan
    
    # Create directory for products
    try {
        az storage fs directory create --account-name $StorageAccountName --file-system bronze --name "dp203/07/products" --auth-mode login
        Write-Host "Created directory: dp203/07/products" -ForegroundColor Green
    }
    catch {
        Write-Error "Error creating products directory: $_"
        # Continue even if there's an error (directory might already exist)
    }
    
    # Upload products.csv file
    Write-Host "Uploading products.csv to Azure Data Lake Storage..." -ForegroundColor Cyan
    $productsFilePath = "$TempPath/data/products.csv"
    
    if (Test-Path $productsFilePath) {
        try {
            az storage fs file upload --account-name $StorageAccountName --file-system bronze --path "dp203/07/products/products.csv" --source $productsFilePath --auth-mode login
            Write-Host "Uploaded products.csv to dp203/07/products" -ForegroundColor Green
        }
        catch {
