...
# This script is intended for lab 04 of the DP-203 course
...
function Download-GitHubFiles {
    param(
        ...
        [string]$SourcePath = "Allfiles/labs/04",
    ...
    # Download CSV files
    $csvFiles = @("customer.csv", "product.csv", "salesorder.csv")
...
if (-not $SkipAzureCopy) {
    ...
    try {
        az storage fs directory create --account-name $StorageAccountName --file-system bronze --name "dp203/04" --auth-mode login
        Write-Host "Created directory: dp203/04" -ForegroundColor Green
    }
    catch {
    ...
    foreach ($file in $csvFiles) {
        ...
        if (Test-Path $filePath) {
            try {
                az storage fs file upload --account-name $StorageAccountName --file-system bronze --path "dp203/04/$file" --source $filePath --auth-mode login
                Write-Host "Uploaded $file to dp203/04" -ForegroundColor Green
            }
            catch {
